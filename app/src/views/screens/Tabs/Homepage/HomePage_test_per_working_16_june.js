...
function HomePage({ route, navigation }) {
  ...
  const getMatchUsersList = async () => {
    ...
    //     images: user.images || [user.profile_image], // Important: Add images array for navigation
    //     name: user.name,
    //     status: 'Online',
    ...
    const newMatchList = response.matches.length > 0 ? response.matches.map((user, index) => {
      const cardName = cardNames[index % cardNames.length];
      const cardType = cardTypes[index % cardNames.length];
      const cardImageKey = 'f_' + cardName;
      return {
  ...
  // Update logic to handle waitingForMatch
  function renderCard(card) {
    if (card.waitingForMatch) {
      return (
        <LottieView
      ...
      return (
        <ImageBackground
          ...
          blurRadius={card.card_type === 'ANONYMOUS' ? 10 : 0}
          source={{ uri: card.images ? card.images[0] : null, cache: 'force-cache' }}
        />
      );
    }
  }
    ...
    <GradientBackground>
      <SafeAreaView style={{ flex: 1 }}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                } else if (activeUser?.waitingForMatch != true) {
                  // navigation.navigate('ViewProfile', {
                  //   current_user: currentUser,
                  //   other_user: activeUser,
                  // });
        ...
        <View ref={matchesGridRef} nativeID="matches-grid" style={{ flex: 1 }}>
          ...
          {/* {NEW WORK } */}
          <TouchableOpacity
            activeOpacity={activeUser?.card_type === 'JOKER' && activeUser?.image === null ? 0.8 : 1}
            onPress={() => {
              // Only handle press for JOKER cards with null image
              if (!activeUser?.card_type === 'JOKER' && activeUser?.image === null) {
                handleJokerCardPress();
              }
            }}
            style={{
              // backgroundColor: 'red',
            ...
            <View style={{ position: 'relative', width: '100%', height: '100%' }}>
              ...
              <Animated.View style={{ opacity: fadeAnim }}>
                ...
                <ImageBackground
                  ...
                  blurRadius={activeUser?.card_type === 'ANONYMOUS' ? 10 : 0}
                  source={
                    activeUser?.card_type === 'JOKER' && activeUser?.image === null
                      ? null // No background image for JOKER cards, show Lottie animation instead
                      : {
                        uri: carouselImages[activeImageIndex] || null,
                ...
                >
                  {/* Lottie animation for JOKER cards with null image */}
                  {activeUser?.card_type === 'JOKER' && activeUser?.image === null && (
                    <LottieView
                      source={require('../../../../assets/lottie/waiting.json')}
                      autoPlay
                  ...
                  {activeUser?.waitingForMatch === true && (
                    <LottieView
                      source={require('../../../../assets/lottie/waiting.json')}
                      autoPlay
                      loop
                  ...
                  {/* TAP TO SEND JOKER text for premium users with JOKER cards */}
                  {activeUser?.card_type === 'JOKER' &&
                    activeUser?.image === null &&
                    activeUser?.name === '' &&
                    (currentUser?.subscription_type === 'silvermonthly12345_new' || currentUser?.subscription_type === 'goldmonthly12345_new') && (
                      <Text
                        style={{
                          position: 'absolute',
                  ...
                  {imageLoading && !(activeUser?.card_type === 'JOKER' && activeUser?.image === null) && !activeUser?.waitingForMatch && (
                    <View
                      style={{
                        position: 'absolute',
                        top: 0,
                  ...
                  >
                    ...
                    >
                      {/* For Joker card with null image, don't show progress indicators */}
                      {activeUser?.card_type === 'JOKER' && activeUser?.image === null ? (
                        // Just a single placeholder bar for joker card
                        null
                      ) : activeDot && activeDot?.images && activeDot?.images?.length > 0 ? (
                        // Dynamic progress bars based on actual images
                        activeDot?.images?.map((_, index) => (
                          <View
                            key={index}
                            style={{
                              flex: 1,
                        ...
                        // Use carousel images for progress indicators
                    ...
                    >
                      <TouchableOpacity
                        activeOpacity={.1}
                        onPress={() => {
                          // Only allow image navigation if not a Joker card with null image
                          if (!(activeUser?.card_type === 'JOKER' && activeUser?.image === null)) {
                            if (activeUser?.images?.length > 1) {
                              handlePrevImage();
                            }
                          }
                        ...
                        style={{
                          width: '50%',
                          height: '100%',
                          opacity: 0, // Make touchable invisible
                        }}
                      >
                      </TouchableOpacity>
                      <TouchableOpacity
                    ...
                    <View>
                      <Text
                        style={{
                          ...
                          alignSelf: 'center',
                          marginTop: responsiveHeight(1),
                          display: activeUser?.card_type === 'JOKER' && activeUser?.image === null
                            || activeUser?.waitingForMatch === true
                            ? 'none' : 'flex'
                        }}
                      >
                        {
                          activeUser?.name ? activeUser?.name : 'New User'}
                      </Text>
                      <Text
                        style={{
                          color: COLORS.white,
                      ...
                      >
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            handleActiveUserInfo(activeUser);
                          }}
                        >
                      ...
                      >
                        ...
                        <TouchableOpacity
                          ...
                          // onPress={() => {
                          //   console.log('current user', JSON.stringify(activeUser, null, 2));
                          //   console.log('Next User Pressed', JSON.stringify(fatelist, null, 2));

                          //   // Fix: Ensure correct matching by checking both keys
                          //   const currentIndex = fatelist.findIndex(
                          //     user => user.id === activeUser.id || user.id === activeUser.user_id
                          //   );

                          //   // Fallback if current user not found
                          //   if (currentIndex === -1) {
                          //     console.warn('Active user not found in fatelist');
                          //     return;
                          //   }

                          //   const nextIndex = (currentIndex + 1) % fatelist.length;
                          //   const nextUser = fatelist[nextIndex];

                          //   console.log('Next User for state', JSON.stringify(nextUser, null, 2));
                          //   // set the next user as active state
                          ...
                          onPress={() => {
                            ...
                            // Check if it's a Joker card with null image
                            if (nextUser.card_type === 'JOKER' && nextUser.image === null) {
                              // For Joker card with null image, set empty images array
                              setCarouselImages([]);
                            } else {
                              // For normal users, set their images
                              setCarouselImages(nextUser.images || []);
                            }
              ...
              {imageLoading &&
                !activeUser?.waitingForMatch &&
                !(activeUser?.card_type === 'JOKER' && activeUser?.image === null) &&  // Don't show loading for Joker cards with null image
                (
                  <View style={{
                    position: 'absolute',
                    top: 0,
